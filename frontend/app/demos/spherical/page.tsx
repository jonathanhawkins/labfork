...
const API_BASE = "/api/demos/spherical";

export default function SphericalDemoPage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await fetch(API_BASE, {
        ...
        body: JSON.stringify({ text, emotion, intensity, interpolate, vad }),
      });
  ...
  return (
    <div className="min-h-screen bg-background p-8">
      <div className="max-w-2xl mx-auto">
        <DemoBreadcrumb demoId="spherical" />
        ...
        <div className="mt-8 text-xs text-muted-foreground">
          <p>Inference: <code className="text-foreground-bright">inference/generate_with_spherical.py</code></p>
        </div>
      </div>
    </div>
  );
