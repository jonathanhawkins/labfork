...
const API_BASE = "/api/demos/promode";

export default function PromodeDemoPage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await fetch(API_BASE, {
        ...
        body: JSON.stringify({ text, visualize }),
      });
    ...
    <div className="min-h-screen bg-background p-8">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="mt-8 text-xs text-muted-foreground">
          <p>Inference: <code className="text-foreground-bright">inference/generate_with_promode.py</code></p>
        </div>
      </div>
    </div>
  );
