...
const API_BASE = "/api/demos/voxgenesis";

export default function VoxgenesisDemoPage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await fetch(API_BASE, {
        ...
        body: JSON.stringify({ text, source, target, edit, sweep, temperature, truncation }),
      });
  ...
  return (
    <div className="min-h-screen bg-background p-8">
      <div className="max-w-2xl mx-auto">
        <DemoBreadcrumb demoId="voxgenesis" />
        ...
        {audioUrl && (
          <div className="p-6 bg-background-card border border-border rounded-lg">
            <div className="flex items-center justify-between mb-4">
              ...
              <a
                href={audioUrl}
                download="voxgenesis_output.wav"
        ...
        <div className="mt-8 text-xs text-muted-foreground">
          <p>Inference: <code className="text-foreground-bright">inference/generate_with_voxgenesis.py</code></p>
        </div>
      </div>
    </div>
  );
