...
const API_BASE = "/api/demos/mismatch_aware";

export default function MismatchawareDemoPage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await fetch(API_BASE, {
        ...
        body: JSON.stringify({ text, emotion, vad, test }),
      });
  ...
  return (
    <div className="min-h-screen bg-background p-8">
      <div className="max-w-2xl mx-auto">
        <DemoBreadcrumb demoId="mismatch_aware" />
        ...
        {audioUrl && (
          <div className="p-6 bg-background-card border border-border rounded-lg">
            <div className="flex items-center justify-between mb-4">
              ...
              <a
                href={audioUrl}
                download="mismatch_aware_output.wav"
        ...
        <div className="mt-8 text-xs text-muted-foreground">
          <p>Inference: <code className="text-foreground-bright">inference/generate_with_mismatch_aware.py</code></p>
        </div>
      </div>
    </div>
  );
