...
const API_BASE = "/api/demos/style_lora";

export default function StyleloraDemoPage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await fetch(API_BASE, {
        ...
        body: JSON.stringify({ text, styles, speaker, temperature, interpolate }),
      });
  ...
  return (
    <div className="min-h-screen bg-background p-8">
      <div className="max-w-2xl mx-auto">
        <DemoBreadcrumb demoId="style_lora" />
        ...
        {audioUrl && (
          <div className="p-6 bg-background-card border border-border rounded-lg">
            <div className="flex items-center justify-between mb-4">
              ...
              <a
                href={audioUrl}
                download="style_lora_output.wav"
        ...
        <div className="mt-8 text-xs text-muted-foreground">
          <p>Inference: <code className="text-foreground-bright">inference/generate_with_style_lora.py</code></p>
        </div>
      </div>
    </div>
  );
