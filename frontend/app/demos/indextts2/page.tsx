...
const API_BASE = "/api/demos/indextts2";

export default function Indextts2DemoPage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await fetch(API_BASE, {
        ...
        body: JSON.stringify({ text, emotion, vector, emotions, profile, intensity, duration, temperature }),
      });
  ...
  return (
    <div className="min-h-screen bg-background p-8">
      <div className="max-w-2xl mx-auto">
        <DemoBreadcrumb demoId="indextts2" />
        ...
        <div className="mt-8 text-xs text-muted-foreground">
          <p>Inference: <code className="text-foreground-bright">inference/generate_with_indextts2.py</code></p>
        </div>
      </div>
    </div>
  );
