...
const INFERENCE_SCRIPT = "inference/generate_with_style_lora.py";
const OUTPUT_DIR = "public/generated";

interface GenerateRequest {
  text: string | number; styles: string | number; speaker: string | number; temperature: string | number; interpolate: string | number;
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const outputFilename = `style_lora_${outputId}.wav`;
    ...
    console.error("style_lora API error:", error);
    return NextResponse.json(
...
async function runInference(
  ...
  return new Promise((resolve) => {
    const args = [
      ...
      "--styles", params.styles.toString(),
      "--speaker", params.speaker.toString(),
