...
const INFERENCE_SCRIPT = "inference/generate_with_eme_tts.py";
const OUTPUT_DIR = "public/generated";

interface GenerateRequest {
  text: string | number; annotated: string | number; emotion: string | number; emphasis: string | number; mode: string | number;
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const outputFilename = `eme_tts_${outputId}.wav`;
    ...
    console.error("eme_tts API error:", error);
    return NextResponse.json(
...
async function runInference(
  ...
  return new Promise((resolve) => {
    const args = [
      ...
      "--annotated", params.annotated.toString(),
      "--emotion", params.emotion.toString(),
      "--emphasis", params.emphasis.toString(),
