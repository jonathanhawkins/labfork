...
const INFERENCE_SCRIPT = "inference/generate_with_voxgenesis.py";
const OUTPUT_DIR = "public/generated";

interface GenerateRequest {
  text: string | number; source: string | number; target: string | number; edit: string | number; sweep: string | number; temperature: string | number; truncation: string | number;
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const outputFilename = `voxgenesis_${outputId}.wav`;
    ...
    console.error("voxgenesis API error:", error);
    return NextResponse.json(
...
async function runInference(
  ...
  return new Promise((resolve) => {
    const args = [
      ...
      "--edit", params.edit.toString(),
      "--sweep", params.sweep.toString(),
      "--temperature", params.temperature.toString(),
      "--truncation", params.truncation.toString(),
      "--output", outputPath,
    ];
