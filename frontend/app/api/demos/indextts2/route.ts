...
const INFERENCE_SCRIPT = "inference/generate_with_indextts2.py";
const OUTPUT_DIR = "public/generated";

interface GenerateRequest {
  text: string | number; emotion: string | number; vector: string | number; emotions: string | number; profile: string | number; intensity: string | number; duration: string | number; temperature: string | number;
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const outputFilename = `indextts2_${outputId}.wav`;
    ...
    console.error("indextts2 API error:", error);
    return NextResponse.json(
...
async function runInference(
  ...
  return new Promise((resolve) => {
    const args = [
      ...
      "--vector", params.vector.toString(),
      "--emotions", params.emotions.toString(),
      "--profile", params.profile.toString(),
