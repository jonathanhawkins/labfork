...
const INFERENCE_SCRIPT = "inference/generate_with_promode.py";
const OUTPUT_DIR = "public/generated";

interface GenerateRequest {
  text: string | number; visualize: string | number;
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const outputFilename = `promode_${outputId}.wav`;
    ...
    console.error("promode API error:", error);
    return NextResponse.json(
...
async function runInference(
  ...
  return new Promise((resolve) => {
    const args = [
      ...
      "--visualize", params.visualize.toString(),
      "--output", outputPath,
    ];
