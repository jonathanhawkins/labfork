...
const INFERENCE_SCRIPT = "inference/generate_with_voxcpm.py";
const OUTPUT_DIR = "public/generated";

interface GenerateRequest {
  text: string | number; streaming: string | number; compile: string | number;
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const outputFilename = `voxcpm_${outputId}.wav`;
    ...
    console.error("voxcpm API error:", error);
    return NextResponse.json(
...
async function runInference(
  ...
  return new Promise((resolve) => {
    const args = [
      ...
      "--streaming", params.streaming.toString(),
      "--compile", params.compile.toString(),
      "--output", outputPath,
    ];
