...
const INFERENCE_SCRIPT = "inference/generate_with_maskgct.py";
const OUTPUT_DIR = "public/generated";

interface GenerateRequest {
  text: string | number; benchmark: string | number; emotion: string | number;
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const outputFilename = `maskgct_${outputId}.wav`;
    ...
    console.error("maskgct API error:", error);
    return NextResponse.json(
...
async function runInference(
  ...
  return new Promise((resolve) => {
    const args = [
      ...
      "--benchmark", params.benchmark.toString(),
