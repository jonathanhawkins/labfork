...
const INFERENCE_SCRIPT = "inference/generate_with_prosody.py";
const OUTPUT_DIR = "public/generated";

interface GenerateRequest {
  text: string | number; csm: string | number; lora: string | number; pitch: string | number; energy: string | number; rate: string | number; intensity: string | number; duration: string | number; temperature: string | number;
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const outputFilename = `prosody_${outputId}.wav`;
    ...
    console.error("prosody API error:", error);
    return NextResponse.json(
...
async function runInference(
  ...
  return new Promise((resolve) => {
    const args = [
      ...
      "--csm", params.csm.toString(),
      "--lora", params.lora.toString(),
      "--pitch", params.pitch.toString(),
      "--energy", params.energy.toString(),
