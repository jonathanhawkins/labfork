...
const INFERENCE_SCRIPT = "inference/generate_with_steering.py";
const OUTPUT_DIR = "public/generated";

interface GenerateRequest {
  text: string | number; emotion: string | number; intensity: string | number; vad: string | number; interpolate: string | number; demo: string | number;
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const outputFilename = `steering_${outputId}.wav`;
    ...
    console.error("steering API error:", error);
    return NextResponse.json(
...
async function runInference(
  ...
  return new Promise((resolve) => {
    const args = [
      ...
      "--vad", params.vad.toString(),
      "--interpolate", params.interpolate.toString(),
      "--demo", params.demo.toString(),
      "--output", outputPath,
    ];
