...
const INFERENCE_SCRIPT = "inference/generate_with_emorl.py";
const OUTPUT_DIR = "public/generated";

interface GenerateRequest {
  text: string | number; emotion: string | number; intensity: string | number; emphasis_words: string | number; emphasis_strengths: string | number; compare: string | number; output_dir: string | number; analyze: string | number;
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const outputFilename = `emorl_${outputId}.wav`;
    ...
    console.error("emorl API error:", error);
    return NextResponse.json(
...
async function runInference(
  ...
  return new Promise((resolve) => {
    const args = [
      ...
      "--emotion", params.emotion.toString(),
      "--intensity", params.intensity.toString(),
      "--emphasis_words", params.emphasis_words.toString(),
      "--emphasis_strengths", params.emphasis_strengths.toString(),
      "--compare", params.compare.toString(),
      "--output_dir", params.output_dir.toString(),
      "--analyze", params.analyze.toString(),
      "--output", outputPath,
    ];
